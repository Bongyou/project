...
String nm=(String)request.getParameter("nm1");
...
try{
	conn=Conn.getConnection();
	String sql="select * from earring where e_name='"+e_name+"'";
	ps=conn.prepareStatement(sql);
	rs=ps.executeQuery();
	while(rs.next()){
		db_eid=rs.getInt(1);
...
try{
	conn=Conn.getConnection();
	String sql="insert into `e_cart` values("+null+",'"+db_eid+"','"+db_id+"','"+date+"',"+num+")";
	ps=conn.prepareStatement(sql);
...
<center>
	<div class="order">
	장바구니 저장이 완료되었습니다.<br><br>
	<button onclick="shoppingCart()">장바구니 이동</button>
	</div>
</center>
<script>
